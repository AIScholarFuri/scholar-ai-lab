...
const BlogSection: React.FC = () => {
  ...
  return <section id="blog" className="py-20 md:py-28 bg-gray-50 relative overflow-hidden">
      <div className="container mx-auto px-4 md:px-8 max-w-7xl">
        <div className="max-w-4xl mx-auto text-center mb-16 reveal" ref={sectionRef}>
          ...
          <p className="text-xl text-gray-600 max-w-3xl mx-auto">Real insights, zero fluff. Read lessons, trends, and ideas from our AI Scholar editors and community.</p>
        </div>
        
        {loading ? <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-8">
            {[...Array(4)].map((_, index) => <div key={index} className="glass-card h-80 animate-pulse">
            ...
            {posts.map((post, index) => <Card key={index} className="glass-card overflow-hidden hover:-translate-y-2 transition-all duration-300 h-full flex flex-col">
                <div className="aspect-video overflow-hidden">
                  <img src={post.thumbnail} alt={post.title} className="w-full h-full object-cover" onError={e => {
              ...
              target.src = 'https://placehold.co/600x400/e0e0e0/818181?text=AI+Scholar';
                ...
                <CardFooter className="px-6 pb-6 pt-0">
                  <a href={post.link} target="_blank" rel="noopener noreferrer" className="text-primary font-medium hover:underline">
                    Read more →
                  </a>
                </CardFooter>
              </Card>)}
        ...
        <div className="mt-12 text-center">
          <a href="https://medium.com/@furiai2025" target="_blank" rel="noopener noreferrer" className="primary-button inline-block">
            View All Articles
          </a>
        </div>
      </div>
